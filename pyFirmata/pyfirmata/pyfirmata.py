...
EEPROM_READ = 0x40
EEPROM_WRITE = 0x41
EERPROM_READ_RESPONSE = 0x42
SERVO_DETACH = 0x43
READ_ANALOG = 0x44
READ_ANALOG_RESPONSE = 0x45
...
class Board(object):
	...
	self.add_cmd_handler(EEPROM_READ,self._handle_report_eeprom)
    ...
    def readEEPROMOnceOpened(self):
	
	addr = 0
	while addr < 127:
		data_send = to_two_bytes(addr)
		self.send_sysex(EEPROM_READ,data_send)
		time.sleep(0.01)
		received_data = []	
		byte = self.sp.read()
		if not byte:
		...
		if data == START_SYSEX:
			...
			if (data == EERPROM_READ_RESPONSE):
				...
				return
			self.EEPROM[addr] = received_data[2] + received_data[3]*128
    ...
    def writeEEPROM(self,addr,val):
  	...
  	data = to_two_bytes(addr)
	data += to_two_bytes(val)
	self.send_sysex(EEPROM_WRITE,data)

    def servoDetach(self,addr):
      
  	data = to_two_bytes(addr)
	self.send_sysex(SERVO_DETACH,data)	
    
    def readAnalogPin(self,addr):

	data_send = to_two_bytes(addr)

	self.send_sysex(READ_ANALOG,data_send)
	time.sleep(0.01)
	received_data = []	
	byte = self.sp.read()
	...
	if data == START_SYSEX:
		...
		if (data == READ_ANALOG_RESPONSE):
			...
			return
		return received_data[1]+received_data[2]*128
    ...
    def _handle_report_eeprom(self, *data):
	pass

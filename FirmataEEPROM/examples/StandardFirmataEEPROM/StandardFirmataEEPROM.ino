...
#define DETACH_SERVO                0x43
#define READ_ANALOG                 0x44
#define READ_ANALOG_RESPONSE        0x45
...
void sysexCallback(byte command, byte argc, byte *argv)
{
  ...
  switch (command) {
    ...
    case EEPROM_READ:
    {
      short addr = argv[0] + (argv[1] << 7);
      byte b = EEPROM.read(addr);
      Firmata.write(START_SYSEX);
      Firmata.write(EERPROM_READ_RESPONSE);
      Firmata.write(argv[0]);
      Firmata.write(argv[1]);
      Firmata.sendValueAsTwo7bitBytes(b);
      Firmata.write(END_SYSEX);
    }
      break;
    case EEPROM_WRITE:
    ...
    case DETACH_SERVO:
    {
      short servo_number = argv[0]+ (argv[1] << 7);
      detachServo(servo_number);
    }
    break;
    case READ_ANALOG:
    {
      short addr = argv[0] + (argv[1] << 7);
      Firmata.write(START_SYSEX);
      Firmata.write(READ_ANALOG_RESPONSE);
      Firmata.sendAnalog(addr, analogRead(addr));
      Firmata.write(END_SYSEX);
    }
  }
}
